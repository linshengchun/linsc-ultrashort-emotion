...
export function RemoteReport({ date, reportsBaseUrl }: { date: string; reportsBaseUrl: string }) {
  const [markdown, setMarkdown] = useState('');
  const [status, setStatus] = useState<'loading' | 'ready' | 'missing' | 'error'>('loading');
  const source = `${reportsBaseUrl}/${date}.md`;
  const pdfSource = `${reportsBaseUrl}/${date}.pdf`;
      ...
      <div className="mx-auto max-w-[1180px]">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          {status === 'ready' ? (
            <div className="flex gap-2">
              <Button asChild variant="outline" className="gap-2"><a href={pdfSource} download><Download className="size-4" />下载PDF</a></Button>
              <Button asChild variant="outline" className="gap-2"><a href={source} download><Download className="size-4" />下载Markdown</a></Button>
            </div>
          ) : null}
        </div>
        ...
        {status === 'missing' ? (
          <div className="rounded-2xl border border-dashed border-amber-300 bg-amber-50 p-8 text-center text-amber-950">
            <h1 className="text-lg font-semibold">{date} 尚未上传静态报告</h1>
            <p className="mt-2 text-sm">请先在本地生成 Markdown/PDF 并上传 GitHub，随后刷新本站。</p>
          </div>
        ) : null}
